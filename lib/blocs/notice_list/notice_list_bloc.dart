...
class NoticeListBloc extends Bloc<NoticeListEvent, NoticeListState> {
  NoticeListBloc({required this.noticeRepository, required this.course}) : super(const NoticeListState(status: NoticeListStatus.loading)) {
  ...
  Future<void> _onFetchNoticeList(Fetch event, Emitter<NoticeListState> emit) async {
    try {
      final Notices notices = await noticeRepository.fetchNoticeList(course.code);
