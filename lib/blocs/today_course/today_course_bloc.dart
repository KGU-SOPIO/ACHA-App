...
class TodayCourseBloc extends Bloc<TodayCourseEvent, TodayCourseState> {
  ...
  Future<void> _onFetchTodayCourses(Fetch event, Emitter<TodayCourseState> emit) async {
    try {
      final TodayCourses todayCourses = await todayCourseRepository.fetchTodayCourses();
      emit(state.copyWith(status: TodayCourseStatus.loaded, todayCourses: todayCourses));
    } on DioException catch (e) {
      emit(state.copyWith(status: TodayCourseStatus.error, message: e.error as String));
    } catch (e) {
      emit(state.copyWith(status: TodayCourseStatus.error, message: '오늘의 강의 정보를 불러오지 못했어요'));
    }
  }
}