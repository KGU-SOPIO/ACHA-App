...
class AlertBloc extends Bloc<AlertEvent, AlertState> {
  ...
  Future<void> _onFetchAlertStatus(
      FetchAlertStatus event, Emitter<AlertState> emit) async {
    try {
      ...
      emit(state.copyWith(
          status: AlertStatus.error, message: e.error as String));
    } on RepositoryException catch (e) {
      emit(state.copyWith(status: AlertStatus.error, message: e.message));
    } catch (e) {
      emit(state.copyWith(
          status: AlertStatus.error, message: '알림 상태를 불러오지 못했어요'));
    }
  }
  ...
  Future<void> _onChangeAlertStatus(
      ChangeAlertStatus event, Emitter<AlertState> emit) async {
